...
class RFIDReader:
    def __init__(self, device_name, event_ready_timeout=EVENT_READY_TIMEOUT):
        self.event_ready_timeout = event_ready_timeout
    ...
    def _read_rfid(self):
        ...
        while keep_reading:
            ...
            keep_reading = self.poller.poll(EVENT_READY_TIMEOUT)
    ...
    def _translate_event(self, event):
        ...
        if isinstance(event, evdev.events.KeyEvent) and event.keystate == evdev.events.KeyEvent.key_down:
            if event.keycode in _KEY_MAP:
                return _KEY_MAP[event.keycode]
