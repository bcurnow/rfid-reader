...
docker-build: ## Build the Docker image
	docker image build \
		...
		--build-arg INPUT_GROUP_ID=$(shell getent group input | cut -d: -f3) \
		-t $(IMAGE_NAME):latest \
		.

docker-run: ## Run a shell in the Docker container (mounts project root and /dev, requires hardware)
	docker run -it \
		--privileged \
